# Криптоанализ шифра ZMAK (Вариант 5)
...
graph TD
    ...
    %% Результат XOR становится новым x0 (обозначим как x0_new)
    XOR_node --> x0_prime[x0']

    %% Связи для остальных элементов (они просто проходят вниз)
    x1 --> x1_out
    x2 --> x2_out
    x3 --> x3_out

    %% Определяем узлы выхода в нужном порядке
...
В ходе тестирования (на фиксированных ключах, заданных в `zmak.h`) атака успешно восстанавливает ключ последнего раунда.
